...
$str11 .= '<option>'. $row[$arrayofcolumn] .  '</option>';
...
function selectFromTable1( $_tableName , $arrayofcolumn="",$arrayofcolumnwhere ="",$arrayofcolumnorderby="" ,$arrayofcolumngroupby="" )
		{
			...
			$this->query=$this->con -> prepare("select $arrayofcolumn from $this->_tableName $str $groupby $orderby ");
			$this->query->execute();
			if(trim($this->_tableName)=="user_login") {				
				$str11 = "";
 $str11 .='<select multiple >';
					while($row=$this->query->fetch())
				{
...
$str11 .= "<option onclick=passdata('$row[$arrayofcolumn]')>". $row[$arrayofcolumn] .  '</option>';
...
return  $str11;
			...
			{
				return  "select $arrayofcolumn from $this->_tableName $str $groupby $orderby ";
